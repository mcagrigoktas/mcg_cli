...
const _escapeKey = 'Quit';

/// Enum listesinden herhangi birini secip onChoose daki islemi yapabilirsiniz. Yada sonucunu herhangi bir degeren esitleyebilirsiniz.
class ChooseOne {
  ChooseOne._();
  static T? run<T extends Enum>(List<T> itemList, {String? hintText, void Function(T)? onChoose, bool useAnsi = true}) {
    if (hintText != null) print('🚧 $hintText');

    final menu = Menu((itemList).map((e) => e.name).toList()..add(_escapeKey), useAnsi: useAnsi).choose();
    if (menu.value == _escapeKey) return null;
    final result = itemList.singleWhere((element) => element.name == menu.value);

    if (onChoose != null) onChoose(result);
    return result;
  }
}
  ...
  _SimpleResult _chooseAnsi() {
    ...
    while (result == null) {
      ...
      for (var i = 0; i < _options.length; i += 1) {
        ...
        _stdout.write('$humanIndex'.padLeft(2));
        _stdout.write(') ');
      ...
      // if (firstEscape == 113 || firstEscape == 27) {
      //   result = null;
      //   break;
      // }
